...
<head>
	<title>Projects | ALGORITHMS AND COMPLEXITY GROUP</title>
...
<!--=== Content Part ===-->
        ...
        <div class="row">
		   <div class="headline headline-center"><h2>PROJECTS</h2></div>
            <a href="add/addprojects.php"><button type="button" class="btn btn-primary pull-right">ADD NEW <i class="fa fa-plus-circle" aria-hidden="true"></i></button></a>		   
            <!-- Begin Easy Block --> 
           <?php
			  include_once 'config/db.php';
			 $stmt = $conn->prepare('SELECT ID, status,title,project FROM projects ORDER BY ID DESC ');
			 $stmt->execute();
			 $sample="x";
			  ...
			  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
			  {
			   ...
			   if($status==$current)
			    ...
			    if($status!=$sample)
				{
                   $sample=$status;					
			?>	
            <div class="headline"><h2><?php echo $status ?></h2></div>
            <?php 
                }
             ?>				
            <div class="container">
			           <h2> <b style="color:#b5111c">TITLE :</b><?php echo $title ?></h2>
				       <p><b style="color:#b5111c">DESCRIPTION :</b><?php echo $project ?></p>  
            </div>
			<?php
				}
			  }
